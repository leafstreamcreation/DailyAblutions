...
class DeterminationViewController<Content> : UIHostingController<Content> where Content : View {
    ...
    override init(rootView: Content) {
        ...
        m_Mantras.append(Mantra.createMany(number: 2, title: "Practice signing", text: "DAD/MOM, BRO/SIS, NEED/MUST, WANT, COOL, NOTHING, AUNT/UNCLE, SEE YOU LATER, HAVE, BOY/GIRL, LANGUAGE, PLACE, FOR, MEAN, FAMILY, CLASS"))
        ...
        m_Mantras.append(Mantra.createMany(number: 5, title: "Car registration", text: "When is the date for the inspection?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "Wall painting", text: "What is the next step finishing wall repairs?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "Network", text: "What are the next steps to grow my network?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "Get in touch with YOU", text: "Do you have a moment to breathe or meditate?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "Cleaning floors", text: "When are you going to finish cleaning the floors?"))
        ...
        m_Mantras.append(Mantra.createMany(number: 5, title: "You are a web developer!", text: "How will you set up a server to send notifications to your phone?"))
        ...
        m_Mantras.append(Mantra.createMany(number: 5, title: "You have a list of repetitive chores", text: "What are they and how will you schedule them?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "You are a good teammate", text: "How will I tackle the dummy answers? What other features do I want to add?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "You have a pudge ball to care for", text: "When are you shaving Chloe's fupa?"))
        m_Mantras.append(Mantra.createMany(number: 5, title: "Manage your environment", text: "When are you terminating Steam?"))
